...
export const config = {
  ...
  worker: {
    ...
    navigationTimeoutMs: positiveInteger(process.env.NAVIGATION_TIMEOUT_MS, 20000, 60000),
    actionTimeoutMs: positiveInteger(process.env.ACTION_TIMEOUT_MS, 10000, 30000),
    discoveryTimeoutMs: positiveInteger(process.env.DISCOVERY_TIMEOUT_MS, 30000, 90000),
    totalTargetTimeoutMs: positiveInteger(process.env.TOTAL_TARGET_TIMEOUT_MS ?? process.env.AUTOMATION_TIMEOUT, 60000, 180000),
    timeoutMs: positiveInteger(process.env.TOTAL_TARGET_TIMEOUT_MS ?? process.env.AUTOMATION_TIMEOUT, 45000, 180000),
    websiteTimeoutMs: positiveInteger(process.env.WEBSITE_TIMEOUT ?? process.env.TOTAL_TARGET_TIMEOUT_MS, 60000, 300000),
    heartbeatIntervalMs: positiveInteger(process.env.HEARTBEAT_INTERVAL_MS, 12000, 30000),
    staleHeartbeatThresholdMs: positiveInteger(process.env.STALE_HEARTBEAT_THRESHOLD_MS, 45000, 120000),
  },
  proxy: {
