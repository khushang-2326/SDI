...
export const config = {
  ...
  worker: {
    maxWorkers: positiveInteger(process.env.MAX_WORKERS ?? process.env.WORKER_CONCURRENCY, 8, 16),
    concurrency: positiveInteger(process.env.MAX_WORKERS ?? process.env.WORKER_CONCURRENCY, 8, 16),
    ...
    totalTargetTimeoutMs: positiveInteger(process.env.WEBSITE_TIMEOUT ?? process.env.TOTAL_TARGET_TIMEOUT_MS ?? process.env.AUTOMATION_TIMEOUT, 150000, 300000),
    timeoutMs: positiveInteger(process.env.WEBSITE_TIMEOUT ?? process.env.TOTAL_TARGET_TIMEOUT_MS ?? process.env.AUTOMATION_TIMEOUT, 150000, 300000),
    websiteTimeoutMs: positiveInteger(process.env.WEBSITE_TIMEOUT ?? process.env.TOTAL_TARGET_TIMEOUT_MS ?? process.env.AUTOMATION_TIMEOUT, 150000, 300000),
